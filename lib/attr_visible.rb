module AttrVisible
  def self.included(base)
    base.cattr_reader :serialize_defaults_options
  ...
  module ClassMethods
    ...
    def serialize_defaults serialize_options
      [:to_xml, :to_json, :to_yaml].each do |to_meth|
        @@serialize_defaults_options          = serialize_options
        @@serialize_defaults_options[:only] ||= self.visible_attributes
        class_eval <<-end_eval
        def #{to_meth}_with_defaults( options = {}, &block)
          self.#{to_meth}_without_defaults(options.reverse_merge(self.class.serialize_defaults_options), &block)
        end
      ...
      def attr_visible(*attributes)
        ...
        serialize_defaults (@@serialize_defaults_options ||= {}).merge( :only => self.visible_attributes )
      end
