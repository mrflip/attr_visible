...
Because "default permit is dumb":http://bit.ly/default_permit_is_dumb,
this module only allows you to whitelist attributes.
...
* The various serialization methods (+to_xml+, +to_json+, etc.) are
  ...
  * Supplying an +only+ option to to_xml and friends will freely override their
    defaults.

  * So will supplying an +only+ option to serialize_defaults.
